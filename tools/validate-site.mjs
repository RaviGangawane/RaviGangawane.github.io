...
const failures = [];
const notices = [];
const ignoredDirectories = new Set([".git", ".lighthouseci", "node_modules", "docs"]);
...
for (const file of htmlFiles) {
  ...
  if (headings.length !== 1) {
    failures.push(`${relative}: expected exactly one h1, found ${headings.length}`);
  }
  ...
  for (const fragment of references.filter((reference) => reference.startsWith("#") && reference.length > 1)) {
...
for (const jsonFile of ["manifest.webmanifest", "package.json", "lighthouserc.json"]) {
  try {
...
for (const script of files.filter((file) => extname(file) === ".js" || extname(file) === ".mjs")) {
  const result = spawnSync(process.execPath, ["--check", script], { encoding: "utf8" });
  if (result.status !== 0) failures.push(`${script.slice(root.length + 1)}: ${result.stderr.trim()}`);
}

const requiredFiles = ["robots.txt", "sitemap.xml", "manifest.webmanifest", "404.html"];
for (const required of requiredFiles) {
  if (!existsSync(join(root, required))) failures.push(`Missing required support file: ${required}`);
}
...
if (logoSize > 100_000) failures.push(`Optimized logo exceeds 100 KB budget (${logoSize} bytes)`);
notices.push(`optimized logo: ${(logoSize / 1024).toFixed(1)} KB`);

const versionsHtml = readFileSync(join(root, "versions", "index.html"), "utf8");
if (versionsHtml.includes("<iframe")) failures.push("Versions page must use static previews instead of iframes");
...
console.log(`✓ Validated ${htmlFiles.length} HTML pages and ${files.length} current-site files`);
